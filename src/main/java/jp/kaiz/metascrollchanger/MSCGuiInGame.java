...
public class MSCGuiInGame extends GuiScreen {
            ...
            if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
                if (Keyboard.isKeyDown(MetaScrollChanger.KEY_TRIGGER.getKeyCode())) {
                    ...
                    for (int i = 0; i < inventorySize; i++) {
                        ...
                        this.renderInventorySlot(list.get(damage), j1, k1, event.getPartialTicks());
                        j1 += 20;
                    }
    ...
    protected void renderInventorySlot(ItemStack itemstack, int p_73832_2_, int p_73832_3_, float p_73832_4_) {
        if (itemstack != null) {
            float f1 = (float) itemstack.getAnimationsToGo() - p_73832_4_;
            ...
            this.mc.getRenderItem().renderItemAndEffectIntoGUI(itemstack, p_73832_2_, p_73832_3_);

            if (f1 > 0.0F) {
                GL11.glPopMatrix();
            }

            this.mc.getRenderItem().renderItemOverlayIntoGUI(this.mc.fontRenderer, itemstack, p_73832_2_, p_73832_3_, "");
        }
    }
}
